...
namespace Sea.Core.Util.MemoryCache
{
    public class RedisDatabase
    {
        ...
        {
            return _db.SetAddAsync(GetKey(key), JsonSerializer.Serialize(obj));
        }

        public Task<bool> SetRemoveAsync<T>(string key, T obj)
        {
            return _db.SetRemoveAsync(GetKey(key), JsonSerializer.Serialize(obj));
        }

        public Task<bool> SetContainsAsync<T>(string key, T obj)
        {
            return _db.SetContainsAsync(GetKey(key), JsonSerializer.Serialize(obj));
        }
        ...
        {
            return _db.SortedSetAddAsync(GetKey(key), JsonSerializer.Serialize(member), score);
        }
        ...
        {
            return _db.SortedSetDecrementAsync(GetKey(key), JsonSerializer.Serialize(member), value);
        }
        ...
        {
            return _db.SortedSetIncrementAsync(GetKey(key), JsonSerializer.Serialize(member), value);
        }
        ...
        public async Task<IList<KeyValuePair<T, double>>> SortedSetRangeByRankWithScoresAsync<T>(string key, long start = 0, long stop = -1, Order order = Order.Ascending)
        {
            ...
            return cache.Select(m => new KeyValuePair<T, double>(JsonSerializer.Deserialize<T>(m.Element), m.Score)).ToList();
        }
        ...
        public async Task<KeyValuePair<T, double>> SortedSetPopAsync<T>(string key, Order order = Order.Ascending)
        {
            ...
            return entry != null ? new KeyValuePair<T, double>(JsonSerializer.Deserialize<T>(entry.Value.Element), entry.Value.Score) : default;
        }
        ...
        {
            return _db.SortedSetRemoveAsync(GetKey(key), JsonSerializer.Serialize(member));
        }
